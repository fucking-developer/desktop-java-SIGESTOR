 ...
 *
 */
public class DialogoMostrarResultadosEliminacionDirectaMarcadores extends JDialog {
	 ...
	 */
	public DialogoMostrarResultadosEliminacionDirectaMarcadores(VentanaPrincipal principal,
			int indiceMostrarResultados) {
		...
		panelCentralCentral.setLayout(new GridLayout(1, 1, 1, 2));
		panelCentralCentral.setBackground(new Color(100, 105, 105));
		panelCentralCentral.setSize(200, 1 * 300);

		JPanel panelFilasTabla[] = new JPanel[1];
		...
		campoTorneo.setBounds(350, 20, 230, 25);
		...
		campoFechaInicio.setBounds(350, 20, 220, 25);
		...
		etiquetaCicloActual= new JLabel("Ronda actual:");
		...
		campoFechaFin = new JTextField("01/02/2023");
		...
		etiquetaElegirCiclo = new JLabel("Elegir Ronda: ");
		 ...
		 * etiquetaTituloTabla.setText("Tabla de resultados de el(la) <ciclo actual>");
		 * } }; accionSeleccionarCiclo.putValue(Action.SHORT_DESCRIPTION, "Seleccione "
		 * + this.ventanaPrincipal.getTorneoActual().getDatosPersonalizacion().
		 * getNombreCiclo(3) + " para ver los resultados");
		 * comboSeleccionarCiclo.setAction(accionSeleccionarCiclo);
		 * comboSeleccionarCiclo.getActionMap().put("seleccionarciclo",
		 * accionSeleccionarCiclo);
		 * comboSeleccionarCiclo.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
		 * .put((KeyStroke) accionSeleccionarCiclo.getValue(Action.ACCELERATOR_KEY),
		 * "seleccionarciclo");
		 */
		JPanel panelTitulo = new JPanel();
		JLabel etiquetaTitulo;
		etiquetaTitulo = new JLabel("Tabla de resultados <Ciclo> # ");
		panelTitulo.add(etiquetaTitulo);
		panelTitulo.setBounds(500, 160, 200, 30);
		...
		/*
		 * JLabel etiquetaTitulosTabla = new
		 * JLabel("Tabla de resultados de el(la) <ciclo actual>"); auxPanel = new
		 * JPanel(); auxPanel.setPreferredSize(new Dimension(70, altoTitulos));
		...
		auxPanel = new JPanel();
		auxPanel.setPreferredSize(new Dimension(180, altoTitulos));
		auxPanel.add(etiquetaTitulosTabla);
		auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		panelEncabezados.add(auxPanel);
		etiquetaTitulosTabla = new JLabel("Marcador");
		auxPanel = new JPanel();
		auxPanel.setPreferredSize(new Dimension(100, altoTitulos));
		auxPanel.add(etiquetaTitulosTabla);
		auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		panelEncabezados.add(auxPanel);
		...
		auxPanel = new JPanel();
		auxPanel.setPreferredSize(new Dimension(140, altoTitulos));
		auxPanel.add(etiquetaTitulosTabla);
		auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		panelEncabezados.add(auxPanel);
		...
		auxPanel = new JPanel();
		auxPanel.setPreferredSize(new Dimension(150, altoTitulos));
		auxPanel.add(etiquetaTitulosTabla);
		auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		panelEncabezados.add(auxPanel);
		/*
		 * if (this.validacionMarcadores) { etiquetaTitulosTabla = new JLabel(
		 * this.ventanaPrincipal.getTorneoActual().getDatosPersonalizacion().
		 * getNombreMarcador(1)); auxPanel = new JPanel(); auxPanel.setPreferredSize(new
		 * Dimension(70, altoTitulos)); auxPanel.add(etiquetaTitulosTabla);
		 * auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		 * panelEncabezados.add(auxPanel);
		 * 
		 * }
		...
		/*
		 * for (int i = 0; i < numeroPartidas + auxImpar; i++) { panelFilasTabla[i] =
		 * new JPanel(); panelFilasTabla[i].setLayout(new FlowLayout(1));
		 * 
		 * etiquetaNumeroInicial[i] = new JLabel(String.valueOf(i + 1)); auxPanel = new
		 * JPanel(); auxPanel.add(etiquetaNumeroInicial[i]);
		 * auxPanel.setPreferredSize(new Dimension(70, 30));
		 * 
		 * panelFilasTabla[i].add(auxPanel);
		 * 
		 * etiquetaParticipanteInicial[i] = new JLabel(); auxPanel = new JPanel();
		 * auxPanel.add(etiquetaParticipanteInicial[i]); auxPanel.setPreferredSize(new
		 * Dimension(300, 30));
		 * 
		 * panelFilasTabla[i].add(auxPanel);
		 * 
		 * if (this.validacionMarcadores) { campoMarcadorInicial[i] = new JTextField();
		 * campoMarcadorInicial[i].setEditable(false);
		 * campoMarcadorInicial[i].setPreferredSize(new Dimension(60, 30)); auxPanel =
		 * new JPanel(); auxPanel.add(campoMarcadorInicial[i]);
		 * auxPanel.setPreferredSize(new Dimension(70, 35));
		 * 
		 * panelFilasTabla[i].add(auxPanel); }
		 * 
		 * etiquetaResultado[i] = new JLabel(); auxPanel = new JPanel();
		 * auxPanel.add(etiquetaResultado[i]); auxPanel.setPreferredSize(new
		 * Dimension(240, 30));
		 * 
		 * panelFilasTabla[i].add(auxPanel);
		 * 
		 * etiquetaNumeroFinal[i] = new JLabel(); auxPanel = new JPanel();
		 * auxPanel.add(etiquetaNumeroFinal[i]); auxPanel.setPreferredSize(new
		 * Dimension(70, 30));
		 * 
		 * panelFilasTabla[i].add(auxPanel);
		 * 
		 * etiquetaParticipanteFinal[i] = new JLabel(); auxPanel = new JPanel();
		 * auxPanel.add(etiquetaParticipanteFinal[i]); auxPanel.setPreferredSize(new
		 * Dimension(300, 30)); //
		 * auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		 * panelFilasTabla[i].add(auxPanel);
		 * 
		 * if (this.validacionMarcadores) { campoMarcadorFinal[i] = new JTextField();
		 * campoMarcadorFinal[i].setEditable(false);
		 * campoMarcadorFinal[i].setPreferredSize(new Dimension(60, 30)); auxPanel = new
		 * JPanel(); auxPanel.add(campoMarcadorFinal[i]); auxPanel.setPreferredSize(new
		 * Dimension(70, 35));
		 * 
		 * panelFilasTabla[i].add(auxPanel); }
		 * 
		...
		/*
		 * if (participantes.size() % 2 != 0) { etiquetaNumeroInicial[numeroPartidas]
	 ...
	 */
	/*
	 * private void accionComboSeleccionarCiclo() { Torneo torneo =
	 * this.ventanaPrincipal.getTorneoActual(); ArrayList<Ciclo> listaCiclos =
	 * torneo.getAlgoritmoTorneo().getCiclos(); ArrayList<Participante>
	 * listaParticipantes = torneo.getListaParticipantes(); Ciclo ciclo =
	 * listaCiclos.get(comboSeleccionarCiclo.getSelectedIndex());
	 ...
	 */
	/*
	 * private void inicializar() { Torneo torneo =
	 * this.ventanaPrincipal.getTorneoActual(); ArrayList<Ciclo> listaCiclos =
	 * torneo.getAlgoritmoTorneo().getCiclos(); ArrayList<Participante>
	 * listaParticipantes = torneo.getListaParticipantes();
	 * mostrarDatosGenerales(torneo);
	 * 
	 * for (Ciclo c : listaCiclos) { this.comboSeleccionarCiclo.addItem(c); }
	 ...
	 * ArrayList<Participante> listaParticipantes) { for (int i = 0; i <
	 * listaEncuentros.size(); i++) { for (Participante p : listaParticipantes) { if
	 * (p.getNumeroParticipante() == this
	 ...
	 * etiquetaNumeroInicial[i].setText(String.valueOf(listaEncuentros.get(i).
	 * getIdParticipanteInicial())); etiquetaParticipanteInicial[i]
	 * .setText(this.obtenerParticipante(listaEncuentros.get(i).
	 * getIdParticipanteInicial()) .getNombreParticipante() + " - " +
	 ...
	 * desactivarColumnasTabla(i);
	 * etiquetaNumeroInicial[i].setText(String.valueOf(listaEncuentros.get(i).
	 * getIdParticipanteFinal())); etiquetaParticipanteInicial[i]
	 * .setText(this.obtenerParticipante(listaEncuentros.get(i).
	 * getIdParticipanteFinal()) .getNombreParticipante() + " - " +
	 * this.torneo.getDatosPersonalizacion().getNombreParticipanteSinEncuentro()); }
	 * else { this.etiquetaNumeroInicial[i]
	 * .setText(String.valueOf(listaEncuentros.get(i).getIdParticipanteInicial()));
	 * this.etiquetaNumeroFinal[i]
	 * .setText(String.valueOf(listaEncuentros.get(i).getIdParticipanteFinal())); if
	 * (p.getNumeroParticipante() ==
	 * listaEncuentros.get(i).getIdParticipanteInicial()) {
	 * this.etiquetaParticipanteInicial[i].setText(p.getNombreParticipante()); } if
	 * (p.getNumeroParticipante() ==
	 * listaEncuentros.get(i).getIdParticipanteFinal()) {
	 * this.etiquetaParticipanteFinal[i].setText(p.getNombreParticipante()); } }
	 * 
	 * } if (this.validacionMarcadores) { this.campoMarcadorInicial[i]
	 ...
	 * )); }
	 * this.etiquetaResultado[i].setText(String.valueOf(listaEncuentros.get(i).
	 * toString())); } }
	 * 
	 ...
	 * this.etiquetaResultado[i].setVisible(false); if (this.validacionMarcadores) {
	 ...
	 */
	/*
	 * private Participante obtenerParticipante(int numeroParticipante) { for
	 * (Participante p : participantes) { if (p.getNumeroParticipante() ==
	 * numeroParticipante) { return p; } } return null; }
	 * 
	 ...
	 */
	/*
	 * private void mostrarDatosGenerales(Torneo torneo) { DateFormat f =
	 ...
	 */
	/*
	 * private void accionBotonExportarResultados() { JFileChooser dialogo = new
	 * JFileChooser(); File archivo = null; dialogo.setDialogTitle("Guardar como");
	 * FileFilter filtro1 = new FileNameExtensionFilter("Archivo CSV", "csv",
	 * "CSV"); dialogo.setFileFilter(filtro1);
	 * 
	 * dialogo.setAcceptAllFileFilterUsed(false); dialogo.setCurrentDirectory(new
	 * File(System.getProperty("user.dir"))); dialogo.setSelectedFile(null);
	 * dialogo.setMultiSelectionEnabled(false); int valor =
	 * dialogo.showSaveDialog(null); if (valor == JFileChooser.APPROVE_OPTION) { try
	 * { archivo = dialogo.getSelectedFile();
	 * this.torneo.getAlgoritmoTorneo().generarReporteParciales(archivo,
	 * comboSeleccionarCiclo.getSelectedIndex()); JOptionPane.showMessageDialog(
	 * null, "El archivo se ha guardado exitosamente.", "Generar reporte de " +
	 * this.torneo
	 * .getDatosPersonalizacion().getNombreCiclo(Personalizacion.MAYUSCULA_SINGULAR)
	 * , JOptionPane.INFORMATION_MESSAGE); } catch (ExcepcionUtilerias e) {
