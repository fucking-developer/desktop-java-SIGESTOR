...
public class DialogoCapturarEliminacionDirectaMarcadores extends JDialog implements WindowListener {
	/**
	 ...
	 */
	public DialogoCapturarEliminacionDirectaMarcadores(VentanaPrincipal principal) {
		...
		auxPanel = new JPanel();
		auxPanel.setPreferredSize(new Dimension(190, altoTitulos));
		auxPanel.add(etiquetaTitulosTabla);
		auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		panelEncabezados.add(auxPanel);
		...
		auxPanel = new JPanel();
		auxPanel.setPreferredSize(new Dimension(65, altoTitulos));
		auxPanel.add(etiquetaTitulosTabla);
		auxPanel.setBorder(BorderFactory.createCompoundBorder(borde, borde));
		panelEncabezados.add(auxPanel);
		...
		accionGuardar.putValue(Action.SHORT_DESCRIPTION,"Guarda el marcador y el resultado de cada partida.Deshabilita la edición de los resultados capturados");
		// if (validacionMarcadores) {
		...
		imagenE = imagenF.getImage().getScaledInstance(20, 20, Image.SCALE_AREA_AVERAGING);
		Action accionCancelarCiclo = new AbstractAction("Cancelar " + "el torneo") {
			private static final long serialVersionUID = 1L;
		...
		accionCancelarCiclo.putValue(Action.SHORT_DESCRIPTION, "Elimina los resultados de " + "");
	...
	private void accionBtnMostrarReporte() {
		new DialogoMostrarResultadosEliminacionDirectaMarcadores(ventanaPrincipal,
